...
export const metadata: Metadata = { title: "Recherche — Admin" };

interface Props {
  searchParams: Promise<{ acte?: string }>;
}

export default async function AdminRecherchePage({ searchParams }: Props) {
  ...
  if (!user || user.role !== "ADMIN") redirect("/citoyen");
  ...
  return (
    <>
      <Header
        ...
        subtitle="Retrouvez n'importe quelle déclaration via son numéro d'acte"
        actions={
          <Link
            href="/admin"
            style={{
      ...
      <div className="db-content animate-fade-up">
        <div style={{ marginBottom: "1.5rem" }}>
          <ActeSearch basePath="/admin" placeholder="Ex: ACT-2026-ABC123..." />
        </div>
              ...
              >
                {resultats.map((d, i) => (
                  <RechercheRow
                    ...
                    officierPrenom={d.officier?.prenom ?? null}
                    officierNom={d.officier?.nom ?? null}
                    statut={d.statut}
        ...
        {!acte && (
          ...
          >
            ...
            >
              Entrez un numéro d&apos;acte pour lancer la recherche.
            </p>
          </div>
        )}
      </div>
